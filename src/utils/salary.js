...
function calculateSalaryFrom(fullSalary) {
  ...
  const deductionInssPerc = percentage(baseINSS, discountINSS).toFixed(2);
  const deductionIrpfPerc = percentage(baseINSS, discountIRPF).toFixed(2);
  const balancePercentage = percentage(baseINSS, netSalary).toFixed(2);
